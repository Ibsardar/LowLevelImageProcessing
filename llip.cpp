...
namespace llip {
    ...
    //  normalize_matrix
    //
    void normalize_matrix(Matrix& m, unsigned int w, unsigned int h) {
    ...
    void gaussian_averaging_filter(Image& img, double sigma) {
        ...
        // 1.c)
        normalize_matrix(mask, mask_size, mask_size);
        ...
        for (unsigned int p=0; p<pixels; p++) {
            ...
            normalize_matrix(mask_to_apply, mask_size, mask_size);
